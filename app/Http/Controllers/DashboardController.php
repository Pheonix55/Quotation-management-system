...
class DashboardController extends Controller
{
    ...
    {
        // $user = Auth::user();
        // $products = Product::where('customer_id', $user->id)->get();
        // $terms = Terms::where('customer_id', $user->id)->get();
    ...
    public function storeQuoteStep1(Request $request)
    {
        try {
            $data = $request->validate([
                ...
                'is_completed' => 'nullable|booleaan',
            ]);
    ...
    {
        $selectedProducts = is_string($request->selected_products)
            ? json_decode($request->selected_products, true)
            : $request->selected_products;
        $product_ids = is_string($request->product_ids)
            ? json_decode($request->product_ids, true)
            : $request->product_ids;
        $quotation = quotations::find($id);
        $quotation->update(['product_ids' => $product_ids]);
        $quotation->save();
    ...
    public function show($id)
    {
        ...
        // dd($quotation->product_ids, $prod_ids, $products);
        return view('quotations.quotation_print', compact('quotation', 'products'));
    }

    public function downloadPdf(quotations $quotation)
    {
        // Load the quotation with relationships
        ...
        return Pdf::view('quotations.quotation_print', compact('quotation', 'products'))
            ->format('a4')
